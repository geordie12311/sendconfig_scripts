"""Python script to clean up unconnected interfaces on Nexus devices"""
import json
import getpass
...
"""Generating CDP neighbor information and local interfaces status"""

def get_cdp(task):
    interfaces_list = []
    interfaces_result = task.run(
        task=send_command, command="show interface brief | json"
    )
...
"""Configuring each interface as switchport and shutting it down"""
